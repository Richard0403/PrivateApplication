package com.richard.stepcount.view.richedit.view;
...
import com.richard.stepcount.utils.SDCardUtil;
...
public class RichTextEditor extends ScrollView {
    ...
    public RichTextEditor(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        EditText firstEdit = createEditText("我是文艺的内容~", dip2px(context, EDIT_PADDING));
...
//     * @param imageUrl
//     */
//    public void insertImage(String imageUrl, int width) {
//        Bitmap bmp = getScaledBitmap(imageUrl, width);
...
//    }
    ...
    public void insertImage(String imageUrl) {
        ...
        if (lastEditStr.length() == 0 || editStr1.length() == 0) {
            ...
            addImageViewAtIndex(lastEditIndex, imageUrl);
        } else {
...
//            }
//
//            addImageViewAtIndex(lastEditIndex + 1, imageUrl);
...
//            lastFocusEdit.setSelection(editStr1.length(), editStr1.length());//TODO
            ...
            lastFocusEdit.requestFocus();
            lastFocusEdit.setSelection(editStr2.length(), editStr2.length());//TODO
        }
        hideKeyBoard();
    }
     ...
     * 在特定位置添加ImageView
     */
    public void addImageViewAtIndex(final int index, String imageUrl) {
        final RelativeLayout imageLayout = createImageLayout();
        DataImageView imageView = (DataImageView) imageLayout.findViewById(R.id.edit_imageView);
        imageView.setScaleType(ImageView.ScaleType.FIT_CENTER);//裁剪剧中
        ImageLoader.getInstance().displayImage(imageUrl, getContext(), imageView);
        imageView.setAbsolutePath(imageUrl);//保留这句，后面保存数据会用
//        imageView.setMinimumHeight(PhoneUtils.getWindowHeight(getContext()));//最小为屏幕的宽度
